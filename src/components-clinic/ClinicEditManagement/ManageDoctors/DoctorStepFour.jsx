...
export function DoctorFour({ onClose, nextStep, prevStep ,clinicuuid , doctoruuid}) {
  ...
  const fetchSpecialty = async ()=>{
      const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/doctor-treatment/get-treatment`,{
        method : "Get",
      ...
      const fetchSelectedSpeciality = async() => {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/doctor-treatment/chose-treatment/${doctoruuid}`,{
          method : "Get",
          headers : clinicHeaders()
        });
    ...
    const onCreateOther = async (doctoruuid) =>{
      debugger;
        const specializationExists = specialty.some(
          ...
          const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/doctor-treatment/create-other`,{
            method : "Post",
            headers : clinicHeaders(),
            body :JSON.stringify({
     ...
     const handleRemove=async (data)=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/doctor-treatment/remove-treatment/${data}`,{
          method : "DELETE",
          headers:clinicHeaders()
        });
  ...
  const onCheck =async (data)=>{
    const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/doctor-treatment/choose-treatment`,{
      method : "Post",
      headers : clinicHeaders(),
      body: JSON.stringify({
        treatmentid: data,
  ...
  const onunChecked = async(id,doctoruuid)=>{
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/doctor-treatment/delete-treatment`,{
        method : "Post",
        headers : clinicHeaders(),
        body:JSON.stringify({
          doctorUuid:doctoruuid,
            ...
            >
              <input
                ...
                className="checkbox-theme w-4 h-4 text-primary-600"
                  checked={selectedspecialty.some(x => x.treatment?.id === data.id)}
