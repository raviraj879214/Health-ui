...
export function PackageStepFour({clinicuuid,packageid}){
    ...
    useEffect(()=>{
     ...
     const fetchSelectedClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-treatment/get-package-treatment/${packageid}`,{
            method : "Get",
     ...
     const fetchClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-treatment/get-treatment`,{
            method : "Get",
            headers : clinicHeaders(),
        });
 ...
 const onCheck =async (data)=>{
       debugger;
    const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-treatment/choose-treatment`,{
      method : "Post",
      headers : clinicHeaders(),
      body: JSON.stringify({
        packageid : packageid,
   ...
   const onunChecked = async(id,clinicuuid)=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-treatment/delete-treatment`,{
          method : "Post",
          headers : clinicHeaders(),
          body:JSON.stringify({
            packageid:id,
    ...
    setOtherButton(true);
        ...
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-treatment/create-other`,{
          method : "Post",
          headers : clinicHeaders(),
          body :JSON.stringify({
            packageid: packageid,
...
<DialogTitle className="flex justify-between items-start mb-4">
  ...
  {/* Treatments */}
  {specializationselected.some(item => item.treatment) && (
    <div>
