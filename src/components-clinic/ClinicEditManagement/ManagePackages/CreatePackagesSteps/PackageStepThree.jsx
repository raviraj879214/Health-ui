...
export function PackageStepThree({clinicuuid,packageid}){
    ...
    useEffect(()=>{
     ...
     const fetchSelectedClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialty/get-package-specialty/${packageid}`,{
            method : "Get",
     ...
     const fetchClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialty/get-specialty`,{
            method : "Get",
            headers : clinicHeaders(),
        });
 ...
 const onCheck =async (data)=>{
       debugger;
    const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialty/choose-specialty`,{
      method : "Post",
      headers : clinicHeaders(),
      body: JSON.stringify({
        specialtyId: data,
   ...
   const onunChecked = async(id,clinicuuid)=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialty/delete-specialty`,{
          method : "Post",
          headers : clinicHeaders(),
          body:JSON.stringify({
            packageId:id
      ...
      const onCreateOther = async (clinicuuid)=>{
        ...
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialty/create-other`,{
          method : "Post",
          headers : clinicHeaders(),
          body :JSON.stringify({
            packageId: packageid,
    ...
    <div className="m-2 flex justify-end">
        ...
        {specializationselected.length === 0 ? (
                <p className="text-center">No speciality </p>
            ) :(
                <div className="border border-gray-300 rounded-lg p-3 w-auto max-h-[180px] overflow-auto flex flex-wrap">
                        {specializationselected.map((item) => (
                            ...
                            {item.specialty?.name || item.suggestedCategory?.name}
                            </span>
                            </div>
                        ))}
            </div>
