...
export function PackageStepTwo({clinicuuid,packageid}){
    ...
    useEffect(()=>{
     ...
     const fetchSelectedClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialization/get-package-specialization/${packageid}`,{
            method : "Get",
     ...
     const fetchClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialization/get-specialization`,{
            method : "Get",
            headers : clinicHeaders(),
        });
 ...
 const onCheck =async (data)=>{
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialization/choose-specialization`,{
      method : "Post",
      headers : clinicHeaders(),
      body: JSON.stringify({
   ...
   const onunChecked = async(id,clinicuuid)=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialization/delete-specialization`,{
          method : "Post",
          headers : clinicHeaders(),
          body:JSON.stringify({
            packageId : id,
      ...
      const onCreateOther = async (clinicuuid)=>{
        ...
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-package-specialization/create-other`,{
          method : "Post",
          headers : clinicHeaders(),
          body :JSON.stringify({
            packageId: packageid,
                ...
                {specializationselected.length === 0 ? (
                        ...
                        <div className="border border-gray-300 rounded-lg p-3 w-auto max-h-[180px] overflow-auto flex flex-wrap">
                                {specializationselected.map((item) => (
                                    <div key={item.id}  className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full m-1">
                                    <span className="text-gray-800 text-sm font-semibold">
                                    {item.specialization?.name || item.suggestedCategory?.name}
                                    </span>
                                    </div>
                                ))}
                    </div>
                                     ...
                                     <div className="border theme-border rounded-lg p-3 w-auto flex flex-wrap max-h-[180px] overflow-auto ">
                                        ...
                                        {specializationselected.map((item, index) => (
                                            ...
                                            <span className="text-white-700 text-sm font-semibold">
                                                {item.specialization?.name || item.suggestedCategory?.name || "test close icon"}
                                            </span>
