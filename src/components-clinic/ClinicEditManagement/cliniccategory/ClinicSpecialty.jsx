...
export function ClinicSpeciality({clinicuuid}){
    ...
    useEffect(()=>{
     ...
     const fetchSelectedClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic-speciality/get-clinic-speciality/${clinicuuid}`,{
            method : "Get",
     ...
     const fetchClinicSpecialization = async()=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic-speciality/get-speciality`,{
            method : "Get",
            headers : clinicHeaders(),
        });
 ...
 const onCheck =async (data)=>{
       debugger;
    const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic-speciality/choose-speciality`,{
      method : "Post",
      headers : clinicHeaders(),
      body: JSON.stringify({
        specialtyId: data,
   ...
   const onunChecked = async(id,clinicuuid)=>{
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic-speciality/delete-speciality`,{
          method : "Post",
          headers : clinicHeaders(),
          body:JSON.stringify({
            clinicUuid:clinicuuid,
     ...
     setOtherButton(true);
        ...
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic-speciality/create-other`,{
          method : "Post",
          headers : clinicHeaders(),
          body :JSON.stringify({
            clinicUuid: clinicuuid,
  ...
  <p className="text-center text-gray-500">No speciality</p>
) : (
  <div className="border border-gray-300 rounded-lg p-3 w-auto max-h-[180px] overflow-auto flex flex-col gap-3">
            ...
            >
              <input
                ...
                className="checkbox-theme w-4 h-4 text-primary-600"
                 checked={specializationselected.some(x => x.specialty?.id === data.id)}
                  onChange={(e) => {
                  if (e.target.checked) {
                    onCheck(data.id);   
                  }
                  else{
                    onunChecked(specializationselected.find(x => x.specialty?.id === data.id)?.id,clinicuuid)
                  }
