...
export default function ProductCard({ data }){
    ...
    return (
        <div className="card a-hover-secondary" onClick={()=> onClickNavigate(data.name,data.uuid)}>
            ...
            <div className="content">
                ...
                <h4 className="h4"><Link href={`/clinics/${data.uuid}/${data.name}`}>{data.title}</Link></h4>
            </div>
        </div>
    )
}