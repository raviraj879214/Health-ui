...
export function ListSpecialty({ trigger, sendData , addData }) {
  ...
  const fetchSpecialtyTypes = async (page) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-procedure/get-procedure?page=${page}&limit=${itemsPerPage}`,
  ...
  const onDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-procedure/delete-procedure/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
