...
export function ListSpecialty({ trigger, sendData , addData }) {
  ...
  const fetchSpecialtyTypes = async (page) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-treatment/get-treatment?page=${page}&limit=${itemsPerPage}`,
  ...
  const onDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-treatment/delete-treatment/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
