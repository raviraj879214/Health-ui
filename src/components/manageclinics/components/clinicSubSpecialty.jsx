...
export function ClinicSubSpecialty({ id }) {
    ...
    const fetchSpecialty=async()=>{
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/get-sub-specialty/${id}`,{
            method : "Get",
    ...
    const fetchClinicSpecialty = async () => {
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/get-clinic-sub-specialty/${id}`, {
            method: "Get",
            headers: await adminHeaders()
        });
 ...
 setButton(true);
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/accept-clinic-sub-specialty/${ids}/${id}`,{
            method : "Get",
            headers : await adminHeaders(),
        });
 ...
 setButton(true);
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/reject-clinic-sub-specialty/${ids}/${id}`,{
            method : "Get",
            headers : await adminHeaders(),
        });
...
setButton(false);
    ...
    const assignSpecialty=async(assignid)=>{
        ...
        setButton(true);
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/assign-clinic-sub-specialty`,{
            method : "Post",
            headers: await adminHeaders(),
            body: JSON.stringify({
                "assignid" : assignid,
    ...
    setButton(true);
         ...
         const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/unassign-clinic-sub-specialty`,{
            method : "Post",
            headers: await adminHeaders(),
            body: JSON.stringify({
                                ...
                                <ul className="p-2 text-sm text-body font-medium mt-1 h-[400px] overflow-auto">
                                    ...
                                    {specialty.map((item)=>(
                                      <li key={item.id} className="flex items-center justify-between p-2 border border-neutral-100 rounded-base transition-colors hover:bg-brand-soft">
                                            ...
                                            {item.clinicsSpecialty?.some(clinic => clinic.clinicUuid === id) ? (
                                                <button
