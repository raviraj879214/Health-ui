...
export function ClinicTreatment({ id }) {
    ...
    const fetchSpecialty=async()=>{
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/get-treatment/${id}`,{
            method : "Get",
    ...
    const fetchClinicSpecialty = async () => {
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-clinic/get-clinic-treatment/${id}`, {
            method: "Get",
            headers: await adminHeaders()
        });
                                ...
                                <ul className="p-2 text-sm text-body font-medium mt-1 h-[400px] overflow-auto">
                                    ...
                                    {specialty.map((item)=>(
                                      <li key={item.id} className="flex items-center justify-between p-2 border border-neutral-100 rounded-base transition-colors hover:bg-brand-soft">
                                            ...
                                            {item.clinicTreatments?.some(clinic => clinic.clinicUuid === id) ? (
                                                <button
