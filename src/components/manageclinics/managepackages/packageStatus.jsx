...
export function PackageStatus({ id, onTrigger }) {
  ...
  const sendAction = async (type, messageKey) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/make-an-action`,
        {
          method: "POST",
          headers: {
            ...(await adminHeaders()),
    ...
    <div className="bg-white border shadow rounded-xl">
      ...
      <div className="p-5">
        <Tabs>
          ...
          {renderActionPanel("Activate Package", "active", "send_active")}
          {renderActionPanel("Deactivate Package", "inactive", "send_inactive")}
