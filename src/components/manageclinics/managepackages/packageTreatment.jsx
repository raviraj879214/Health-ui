...
export function PackageTreatment({ id }) {
    ...
    const fetchSpecialty=async()=>{
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/get-treatment/${id}`,{
            method : "Get",
    ...
    const fetchClinicSpecialty = async () => {
        debugger;
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/get-package-treatment/${id}`, {
            method: "Get",
            headers: await adminHeaders()
        });
    ...
    const acceptRequested = async(ids)=>{
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/accept-package-treatment/${ids}/${id}`,{
            method : "Get",
            headers : await adminHeaders(),
        });
    ...
    useEffect(()=>{
     ...
     const rejectRequested = async(ids)=>{
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/reject-package-treatment/${ids}/${id}`,{
            method : "Get",
            headers : await adminHeaders(),
        });
...
setButton(false);
    ...
    const assignSpecialty=async(assignid)=>{
       setButton(true);
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/assign-package-treatment`,{
            method : "Post",
            headers: await adminHeaders(),
            body: JSON.stringify({
                "assignid" : assignid,
    ...
    setButton(true);
         ...
         const res = await fetch(`${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/manage-packages/unassign-package-treatment`,{
            method : "Post",
            headers: await adminHeaders(),
            body: JSON.stringify({
                ...
                <div className="border theme-border rounded-2xl bg-white dark:bg-neutral-900 shadow-sm w-full">
                    <div className="flex items-center justify-between px-5 py-3 border-b theme-border">
                        <Label className="text-sm font-semibold text-heading">
                            Selected ({specialties.filter(item => item.treatment?.name).length})
                        </Label>
                                ...
                                <ul className="p-2 text-sm text-body font-medium mt-1 h-[400px] overflow-auto">
                                    ...
                                    {specialty.map((item)=>(
                                      ...
                                      <li key={item.id} className="flex items-center justify-between p-2 border border-neutral-100 rounded-base transition-colors hover:bg-brand-soft">
                                            ...
                                            {item.packageTreatments?.some(clinic => clinic.packageId === id) ? (
                                                <button
                    ...
                    <div className="p-5 min-h-[120px] text-sm text-body border border-neutral-200 rounded-2xl bg-white">
                            ...
                            {specialties.filter(item => item.treatment?.name).length === 0 ? (
