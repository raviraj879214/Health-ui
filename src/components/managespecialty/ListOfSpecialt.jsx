...
export function ListOfSpecialtiesType({ trigger, sendData }) {
  ...
  const fetchSpecialtyTypes = async (page) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/specialties-type/get-all?page=${page}&limit=${itemsPerPage}`,
  ...
  const onDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_NODEJS_URL}/v1/api/specialties-type/delete-name/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
